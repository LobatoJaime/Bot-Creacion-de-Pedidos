...
size_factor = 0.5
size = [int(img.shape[1] * size_factor), int(img.shape[0] * size_factor)]
cv.imshow('Input Image', cv.resize(img, size, interpolation=cv.INTER_AREA))
...
cv.imshow('Output Image', cv.resize(img, size, interpolation=cv.INTER_AREA))
cv.waitKey(0)
